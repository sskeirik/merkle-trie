...
pub fn find_first_distinct_bits(a: &[u8], b: &[u8], offset: usize, a_bits: usize, b_bits: usize) -> Option<BitDiff> {
    ...
    while i + 8 <= n {
        ...
        if v != 0 {
            ...
            return Some(BitDiff { index: i + extra_bytes, bits: trailing % 8, prefix: false });
        }
        i += 8;
    }
...
impl<const K: usize> BitSeqOps<K> {
    ...
    #[instrument(skip_all)]
    pub fn write_aligned_prefix<A: Allocator + Copy + Debug>(diff: &BitDiff, src: &[u8], alloc: A) -> Box<[u8],A> {
        debug_assert!(!diff.prefix, "this operation is invalid for bitstrings without a diff");
    ...
    #[instrument(skip_all)]
    pub fn write_aligned_suffix<A: Allocator + Copy + Debug>(diff: &BitDiff, src: &[u8], alloc: A) -> Box<[u8],A> {
        debug_assert!(!diff.prefix, "this operation is invalid for bitstrings without a diff");
        let BitDiff { index, bits, .. } = diff;
...
mod test {
    ...
    fn test_find_first_distinct_bits_diff() {
        ...
        fn mk_bit_split<const K: usize>(diff: &BitDiff, a: &[u8], b: &[u8]) -> BitSplit {
            ...
            let (mask, a_i, b_i) = if ! diff.prefix {
            ...
            (diff.prefix, mask, p, a_s, b_s, a_i, b_i)
        }
        ...
        for (idx, (in1, in2)) in inputs.iter().enumerate() {
            ...
            let bit_diff = find_first_distinct_bits(in1, in2, 0, in1.len()*8, in2.len()*8);
    ...
    fn test_find_first_distinct_bits_prefix() {
        ...
        for (a,b, a_bits) in inputs.into_iter() {
            let diff = find_first_distinct_bits(a, b, 0, a_bits, b.len() * 8).expect("failed to find prefix");
